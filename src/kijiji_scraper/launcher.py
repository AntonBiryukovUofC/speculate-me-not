...
def parse_args():
    ...
    parser.add_argument('--init', '--setup',
                        help="Create config file if doesn't exist and open with default text editor",
                        action='store_true')
    parser.add_argument('--conf', '-c', metavar='File path',
                        help="""The script * must read a configuration file to set mail server settings *. Default config file config.yalm is located in ~/.kijiji_scraper/ (MacOS/Linux), APPDATA/.kijiji_scraper (Windows) or directly in the install folder.""")
    ...
    parser.add_argument('--skipmail', '-s',
                        help="Do not send emails. This is useful for the first time you scrape a Kijiji as the current ads will be indexed and after removing the flag you will only be sent new ads.",
                        action='store_true')
    parser.add_argument('--all', '-a', help="Consider all ads as new, do not load ads.json file", action='store_true')
    parser.add_argument('--ads', metavar="File path",
...
def main():
    ...
    if not args.all:
        if args.ads:
            ...
            if not ads_filepath:
                ads_filepath = find_file(['HOME', 'XDG_CONFIG_HOME', 'APPDATA'], ['.kijiji_scraper/ads.json'],
...
"""
    ...
    filepath = find_file(['HOME', 'XDG_CONFIG_HOME', 'APPDATA'], [".kijiji_scraper/config.yaml"],
                         default_content=default_config, create=True)
    # Open with editor
    if os.name == 'nt':
        os.system(filepath)
